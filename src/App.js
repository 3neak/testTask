...
class App extends Component {
	...
	handleChange(event) {
		let inpSearch = event.target.value

		this.setState({ search: inpSearch, filteredData: this.handleFilterData(inpSearch) })
	}

	handleFilterData(value) {
		let newData = this.state.data.filter(el => el.includes(value))
	...
	componentDidMount() {
		...
		if (searchParam !== undefined && searchParam !== null && searchParam?.length !== 0) {
			newData = this.handleFilterData(searchParam)
			search = searchParam
		}

		this.setState({ filteredData: newData, search })
	}

	render() {
		return (
			<Container>
				<Input type="search" onChange={this.handleChange} value={this.state.search} />
				<List>
					{
						this.state.filteredData.map((el, key) => (<Elem key={key}>{el}</Elem>))
					}
				</List>
			</Container >
		)
